...
<div class="mb-6 space-y-4" wire:key="bridge-posts-{{ $user->id }}">
    @foreach ($bridgePosts as $post)
        ...
        <div class="overflow-hidden rounded-md border border-score-400/40 dark:border-score-900" wire:key="bridge-post-{{ $post->id }}">
            <div class="flex items-center gap-2 bg-score-50 px-4 py-2 dark:bg-score-950/40">
                <flux:icon.arrows-right-left class="size-4 text-score-600 dark:text-score-400" />
                <span class="text-sm font-medium text-score-700 dark:text-score-400">{{ __('Bridge Post') }} — {{ $post->theme }}</span>
            </div>
