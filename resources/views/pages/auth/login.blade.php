<x-layouts::auth :title="__('Log in')">
    <div x-data="{ passkeyError: null }" class="flex flex-col gap-6">
        ...
        <template x-if="passkeyError">
            <flux:callout variant="danger" icon="exclamation-triangle" x-bind:heading="passkeyError" />
        </template>

        <a href="{{ route('social.redirect', 'google') }}" class="w-full">
            <flux:button variant="ghost" class="w-full" data-test="login-with-google-button">
        ...
        @if (\Laravel\Fortify\Features::enabled(\Laravel\Fortify\Features::passkeys()))
            <flux:button
                ...
                icon="finger-print"
                class="w-full"
                x-on:click="passkeyError = null; window.Passkeys.loginWithPasskey().catch((e) => passkeyError = e.message)"
