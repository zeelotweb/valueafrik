...
return [
    ...
    'backup' => [
        ...
        'source' => [
            'files' => [
                 ...
                 */
                'include' => [
                    // Only user-uploaded media, not the whole codebase — the
                    // app itself is already versioned in git, and zipping it
                    // into every backup would also capture .env's secrets
                    // inside the archive.
        ...
        'destination' => [
             ...
             */
            // CM_DEFAULT throws "ZipArchive::close(): Invalid argument" on
            // PHP 8.4's bundled libzip (1.11.x) — a known spatie/laravel-backup
            // incompatibility, reproduced locally. CM_DEFLATE is what
            // CM_DEFAULT resolves to anyway, just spelled out explicitly.
         ...
         */
        // An empty-but-set BACKUP_ARCHIVE_PASSWORD (what .env.example ships)
        // is a non-null empty string, not null — which silently turned on
        // zip encryption with a blank password and corrupted the archive on
        // close ("ZipArchive::close(): Invalid argument"), reproduced locally.
    ...
    'notifications' => [
        ...
        'mail' => [
            // env() only falls back on an *unset* key, not an empty one, and
            // .env.example ships this blank — so use ?: to cover both.
