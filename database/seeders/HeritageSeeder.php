...
class HeritageSeeder extends Seeder
{
     ...
     *
     * Grouped by region — the same six buckets Culture Sprint's "region I'm
     * curious about" filter offers, so every heritage here is reachable by it.
     *
    ...
    {
        foreach (self::HERITAGES as $region => $names) {
            foreach ($names as $name) {
                Heritage::updateOrCreate(
                    ['slug' => Str::slug($name)],
